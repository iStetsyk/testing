...
class DayCalkulator {
    ...
    public static void main(String[] args) {
        ...
        int[] monthLengthInNormalYear = {0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};

        for (int i = birthDay + 1; i < todayYear; i++) {
            sum += isLeapYear(i) ? 366 : 365;
        }

        for (int i = birthMonth + 1; i <= 12; i++) {
            if (isLeapYear(birthDay) && i == 2) {
                sum += monthLengthInNormalYear[i] + 1;
            } else {
